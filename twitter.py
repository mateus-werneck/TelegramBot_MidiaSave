...
####################################### Twitter Video #####################################################
def getTwitter(url, firefox):
    temp_dir = tempfile.mkdtemp()
    directory_final = 'C:\\Users\\Mateus\\Documents\\GitHub\\telegram_bot'
    firefox.get(url)
    time.sleep(5)
    ...
    if (re.search(r'https://video.twimg.com/(.*?).m3u8', video) != None):
        ...
        if (m3u8c == None):
            tipo='m4s'
            urllib.request.urlretrieve(video, temp_dir + '\\temp.txt')
            with open (temp_dir + "\\temp.txt", "r") as tempF:
                for line in tempF:
            ...
            if(tipo == 'ts'):
                ...
                twitter.to_videofile(final_name, threads=8, preset="veryfast", audio_codec='aac')
            ...
            with open (temp_dir + "\\temp.txt", "r") as tempF:
                for line in tempF:
                    if(re.search(r'.m3u8', line) != None):
                        ...
                        break
            tempF.close()
            urllib.request.urlretrieve(m3u8c2, temp_dir + '\\temp.txt')
            with open (temp_dir + "\\temp.txt", "r") as tempF:
                for line in tempF:
                    if(re.search(r'.ts', line) != None):
                        encoded.append('https://video.twimg.com' + line)
